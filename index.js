...
var analyzeMonthlyTransactions = (done) => {
  ...
  months.forEach((month) => {
    ...
    transactions.forEach((transaction) => {
      if (transaction.category) {
        if (transaction.category.includes('Food & Dining')) {
          ...
          merchants.push(transaction.merchant);
        } else {
          if (!transaction.category.includes('Travel->Air Travel') && !transaction.category.includes('Travel->Hotel')) {
            discretionaryAmount += new Number(transaction.amount);
            merchants.push(transaction.merchant);
          }

          if (transaction.category.includes('Gift')) {
             giftAmount += new Number(transaction.amount);
             merchants.push(transaction.merchant);
           }
        }
      }
    ...
    console.log('Total spent: €' + Math.round(discretionaryAmount + foodAmount));
    ...
    console.log(`  Non-gifts: €${Math.round(discretionaryAmount - giftAmount)}`);
    console.log(`  Gifts:     €${Math.round(giftAmount)}`);
    console.log(`Total food amount: €${Math.round(foodAmount)} (${Math.round(foodAmount / budgets['food'] * 100)}%)`, '\n');
  });

  //console.log('Merchants', _.uniq(merchants).join('\n'));
